...
class VCS(HammerSimTool, SynopsysTool):
    ...
    def fill_outputs(self) -> bool:
        ...
        self.output_saifs = []
        if self.get_setting("sim_type") == "none":
            self.sim_type = "sim"
        else
            self.sim_type = self.get_setting("sim.inputs.sim_type")
        ...
        self.campaign_tb_dut = self.get_setting("sim.inputs.campaign_tb_dut")
        self.campaign_tcl = self.get_setting("sim.inputs.campaign_tcl")
    ...
    @property
    def run__fsim_tcl_path(self) -> str:
        return os.path.join(self.run_dir, "fsim.tcl")

    @property
    def fault_report_path(self) -> str:
        return os.path.join(self.run_dir, self.get_setting("sim.inputs.output_fault_rpt"))
    ...
    def run_fsim_vcs(self) -> bool:
        if(self.sim_type == "fsim")
            ...
            access_tab_filename = self.access_tab_file_path
            tb_name = self.get_setting("sim.inputs.tb_name")
            strobe_file_name = self.get_setting("sim.inputs.strobe_file_name")
    ...
    def fgen(self) -> bool:
        if(self.sim_type == "fsim")
            fcc_bin = self.get_setting("sim.vcs.vc_fcc_bin")
            if not os.path.isfile(fcc_bin):
                self.logger.error("VC Z01X binary not found as expected at {0}".format(fcc_bin))
                return False
            campaign_simv_daidir = self.get_setting("sim.inputs.campaign_simv_daidir")
            ...
            "-sff config.sff",
            "gen_{self.campaign_tb_dut}.sff",
            "-report fault_report.rpt",
            "-campaign {self.campaign_tb_dut}",
            "-collapse off"
            ]
            ...
            return os.path.exists(fault_report_path)
        else 
            return True
    ...
    def fcc(self) -> bool:
        if(self.sim_type == "fsim")
            ...
            fcc_bin,
            "-full64",
            "-daidir {self.campaign_simv_daidir}", 
            "-sff config.sff",
            "fault_report.rpt",
            "-campaign {self.campaign_tb_dut}",
            "-overwrite"
            ...
            return os.path.exists(fdb_path)
        else 
            return True

    def fcm(self) -> bool:
        if(self.sim_type == "fsim")
            fcm_bin = self.get_setting("sim.vcs.vc_fcm_bin")
            ...
            return os.path.exists(run_tcl_path)
        else
            return True
