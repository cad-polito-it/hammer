...
class VC_ZOIX(HammerFaultSimTool, SynopsysTool):
    ...
    def _read_slack_info(self, input_file: str) -> dict:
        fault_placement = {}
        # Pattern to capture rise, fall, and the point name
        pattern = r'^\s*([-+]?\d*\.?\d+|\*)\s+([-+]?\d*\.?\d+|\*)\s+(.*)$'
        
        # Use a default period if '*' is found
        default_period = float(self.clocks[0].get('period'))
        with open(input_file, "r") as fin:
            for line in fin:
                ...
                if match:
                    ...
                    key = f"{self.top_module}.{point.replace('/', '.')}"
    ...
    def _fuse_slack_with_sff(self, slack_file_path: str, fault_list_path: str) -> bool:
        ...
        fault_placement = self._read_slack_info(slack_file_path)
        ...
        pattern = r'\s*<\s*([\d\s]+)\s*>\s*(\w+)\s*([RF])\s*\{PORT\s+"([^"]+)"\}'
        ...
        with open(fault_list_path, "r") as fin:
            for line in fin:
                ...
                if match:
                    fault_info, status, value, port_raw = match.groups()
                    
                    if port_raw in fault_placement:
                        delay = nets[port_fixed][value]
                        timing_str = f"({delay}{clock_time_unit})"
                        # Rebuild line: note the double {{ }} for literal braces
                        new_line = f"  <{fault_info}> {status} {value} {timing_str} {{PORT \"{port_raw}\"}}\n"
    ...
    def run_vcs(self) -> bool:
        ...
        if self.level.is_gatelevel():
            ...
            if self.get_setting("fsim.inputs.timing_annotated"):
                ...
                if self.sdf_file:
                    args.extend(["-sdf" , self.sdf_file ])
                # TODO MAYBE HERE WE NEED TO PATH THE TOP MODULE SINCE THE SDF_PATH IS NYI 
                # or delay_mode_distributed_path 
