...
class TESTMAX(HammerATPGTool, SynopsysTool):
    ...
    def run_atpg(self) -> bool:
        ...
        if self.create_patterns:
            ...
            self.generated_ud_faults_path = os.path.join(self.result_dir, f'{self.top_module}_ud.flt')
            ...
            # Save the undetectable (UD) faults from this ATPG run so a later
            # fault simulation of a re-graded or modified pattern set can restore
            # them and keep the reported test coverage realistic.
            self.append(f'# save undetectable faults for later fault-sim regrading')
            self.append(f'write_faults {self.generated_ud_faults_path} -class ud -replace')
    ...
    def run_fault_sim(self) -> bool:
        ...
        if patterns_source:
            ...
            self._load_faults(restore_ud=True)
    ...
    def _load_faults(self, restore_ud: bool = False) -> None:
        """Append TCL commands to set up the fault list.

        If ``atpg.inputs.faults_file`` (the incremental ATPG fault list) is
        provided, reads faults from that file with ``-force_retain_code``.
        Otherwise, adds all faults (excluding fakeram modules).

        When ``restore_ud`` is set (fault-simulation context), the undetectable
        (UD) faults from the original ATPG run are additionally restored with
        ``read_faults -retain`` so the reported test coverage of a re-graded or
        modified pattern set stays realistic. The UD list (``atpg.inputs.ud_faults_file``,
        or the one generated by this run) is a separate input from ``faults_file``.
        """
        ...
        if restore_ud:
            # UD list, its own input, independent of faults_file: an explicit
            # atpg.inputs.ud_faults_file, else the one this run just generated.
            ud_faults = self.get_setting("atpg.inputs.ud_faults_file", nullvalue=None)
            if not ud_faults and self.did_generate_patterns:
                ud_faults = self.generated_ud_faults_path
            if ud_faults:
                self.append(f'# restore UD faults from the original ATPG run for accurate test coverage')
                self.append(f'read_faults {ud_faults} -force_retain_code')
            elif self.patterns_source_kind == "user":
                # No UD list: the faults the original ATPG proved undetectable
                # are counted as if detectable, so test coverage looks pessimistic.
                # faults_file is the incremental ATPG list, not a UD list, so it
                # does not substitute here.
                self.logger.warning(
                    "Fault simulating a user-supplied pattern set without the "
                    "undetectable faults from the original ATPG run; reported test "
                    "coverage may be pessimistic. Provide atpg.inputs.ud_faults_file "
                    "(saved by that run with 'write_faults <file> -class ud')."
                )
    ...
    def _generate_reports(self, mode: str) -> bool:
        ...
        report_faults_ud = os.path.join(base_dir, f'{self.top_module}{suffix}_ud_faults.fau')
        self.append(f"write_faults {report_faults_ud} -class ud -replace")
